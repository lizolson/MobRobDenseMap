...
def main():
  ...
  for file in os.listdir(leftdir):
    ...
    for c in range(width):
        for r in range(height):
            ...
            if d > 20:#20
                ...
                # out = myH.dot(homg)
                out = np.copy(homg)
                # cloud[r, c, :] = [out[0,0], out[1,0], out[2,0]]
                cloud[c * height + r] = [out[0, 0], out[1, 0], out[2, 0]]
                color = leftimg[r, c] / 255.0
                # colors[r, c, :] = [color[2],color[1] ,color[0]]
    ...
    # pcd.points = Vector3dVector(np.reshape(cloud,[height*width,3]))
    # pcd.colors = Vector3dVector(np.reshape(colors,[height*width,3]))
